...
class ORM {
  ...
  async fetchHasMany(relatedModelName, foreignKey, instanceId) {
    const builder = new QueryBuilder(relatedModelName);
    builder.where(foreignKey, '=', instanceId);
    const query = builder.buildSelect();
    return await this.connection.query(query, [instanceId]);
  }

  async fetchBelongsTo(relatedModelName, primaryKey, instanceId) {
    const builder = new QueryBuilder(relatedModelName);
    builder.where(primaryKey, '=', instanceId);
    const query = builder.buildSelect();
    return await this.connection.query(query, [instanceId]);
  }
  ...
  async insert(table, data) {
    ...
    const result = await this.connection.query(`${query} RETURNING *`, values);
    return result;
  }
